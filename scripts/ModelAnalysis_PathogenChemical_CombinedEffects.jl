...
treatment_ids_chem = Dict(zip(
    [0, 0.03, 0.3, 3., 30.],
    collect(eachindex([0, 0.03, 0.3, 3., 30.]))
))
...
function define_defaultparams_combinedeffects()::ComponentVector
    ...
    posterior_summary_TKTD = CSV.read(datadir("sims", "$(SAVETAG_TKTDFIT)_$(PMOA_24D)", "posterior_summary.csv"), DataFrame)
