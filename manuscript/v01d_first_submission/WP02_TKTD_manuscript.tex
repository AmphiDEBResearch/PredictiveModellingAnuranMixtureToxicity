...
%
%\affiliation[UOS]{Institute of Mathematics, Universität Osnabrüuck}
...
\def\firstAuthorLast{Hansul {et~al.}} %use et al only if is more than 1 author
\def\Authors{Simon Hansul\,$^{1,*}$, Samuel González-López\,$^{2}$ Manuel E. Ortiz-Santaliestra\,$^{2}$, Jean Lou CM Dorne,$^{3}$, Andreas Focks,$^1$}
% Affiliations should be keyed to the author's name with superscript numbers and be listed as follows: Laboratory, Institute, Department, Organization, City, State abbreviation (USA, Canada, Australia), and Country (without detailed address information such as city zip codes or street names).
% If one of the authors has a change of address, list the new address below the correspondence details using a superscript symbol and use the same symbol to indicate the author in the author list.
\def\Address{$^{1}$Institute of Mathematics, Universität Osnabrück, Germany \\
$^{2}$Institute for Game and Wildlife Research, (IREC) UCLM-CSIC-JCCM, Ciudad Real, Spain \\
$^{3}$European Food Safety Authority, Parma, Italy
}
% The Corresponding Author should be marked with an asterisk
% Provide the exact contact address (this time including street name and city zip code) and email of the corresponding author
\def\corrAuthor{gaiac eco assessment GmbH, An der Ölmühle 2, 52074 Aachen, Germany}

\def\corrEmail{hansul@gaiac-eco.de}

% additional packages and settings used for this manuscript
\usepackage{tabularx}
\usepackage{makecell}
...
\title[Predictive modelling mixture toxicity]{Predictive modelling of chemical mixture toxicity to amphibians in \textit{Discoglossus galganoi}
...
We calibrated the AmphiDEB-TKTD model to single-substance toxicity data for Flupyradifurone and 2,4-D in \textit{Discoglossus galganoi} larvae, 
...
which qualitatively differ in the severity of their effects, regarding the propagation to different apical endpoints. 
...
\tiny
 \keyFont{ \section{Keywords: Dynamic Energy Budgets, Mechanistic Effect Modelling, Mixture toxicity, Pesticides, Amphibians} } %All article types: you may provide up to 8 keywords; at least 5 are mandatory.
\end{abstract}
...
While sensitivity of amphibian larvae to chemicals is often well correlated to that of fish \citep{weltjeCOMPARATIVEACUTECHRONIC2013}, 
this does not apply to all chemicals \citep{glabermanEvaluatingRoleFish2019}, especially following long-term exposures.
...
DEB models can be extended to account for the idiosyncracies of the amphibian life cycle 
\citep{romoliDynamicEnergyBudget2025,hansulDynamicEnergyBudget2026} and
effects of chemicals can be incorporated into DEB models by coupling them to a toxicokinetic-toxicodynamic (TKTD) submodel 
via physiological modes of action (PMoA). 
...
\textit{A priori}, there is no good reason to deviate from the standard approach for the application to amphibians. 
In general, it can be necessary to include other PMoAs to explain observed effects, 
...
%
The DEB-TKTD model was derived from on DEBkiss \citep{jagerDEBkissQuestSimplest2013} 
...
\begin{align}
    embryo &= 
    \begin{cases}
        \text{true} & \text{if } X^{emb}_{int} > 0 \\
        \text{false} & \text{otherwise}
    \end{cases} \\
    larva &= 
    \begin{cases}
        \text{true} & \text{if } X^{emb}_{int} = 0 \land H < H^j \\
        \text{false} & \text{otherwise}
    \end{cases}\\
    metamorph &= 
    \begin{cases}
        \text{true} & \text{if } H \geq H^j \land E^{mt}>0 \\ 
        \text{false} & \text{otherwise}
    \end{cases}\\
    juvenile &= 
    \begin{cases}
        \text{true} & \text{if } H \geq H^j \land E^{mt}=0 \land H < H^p \\ 
        \text{false} & \text{otherwise}
    \end{cases} \\
    adult &= 
    \begin{cases}
        ...
        \text{false} & \text{otherwise}
    \end{cases}
    \label{eq:lifestages_amphibians}
\end{align}%
%
Symbols for the parameters and state variables of the baseline model are listed in Table \ref{tab:symbols_deb}.
...
Baseline parameter values used in this study are taken from \citet{hansulDynamicEnergyBudget2026} and reported in Table \ref{tab:params_discoglossus}. 
...
of an additional weight parameter \citep{jagerBiologybasedApproachMixture2010}. This means that the data is either fitted to the mixture data - 
which would contradict the goals of the present study - or that all TKTD parameters have to be estimated simultaneously 
from the combined single-stressor data, as previously done for lethal effect modelling \citep{bartPredictingMixtureEffects2021}. 
The assumption of IA is therefore a plausible and practical starting point.
%
The model was implemented in the Julia programming language. 
A copy of the code repository is available as supporting information.
%
\subsubsection{Parameter inference}
%
TKTD parameters were inferred from single-stressor effect data of 2,4-D and Flupyradifurone \citep{gonzalez-lopezChronicExposurePesticides2026}.
For details on data generation and statistical analysis, see \citet{gonzalez-lopezChronicExposurePesticides2026}. 
Chemical stressors were applied in their commercial formulations Primma \textregistered Dos and Sivanto \textregistered Prime, respectively.  \\ 
The calibration data consisted of time-resolved larval wet mass, 
the time to reach Gosner stage (GS, \citet{gosnerSimplifiedTableStaging1960}) 42 and the wet mass at GS 42. 
The time to reach GS 42 was used as the time-resolved fraction 
of tadpoles, calculated as the number of survivors which had not yet reached GS 42, 
relative to the total number of surivors at the given time point.
A slower decrease in this value over time therefore indicates delayed metamorphosis, 
...
using the Euclidean distance as distance function, which is a default choice in ABC \citep{prangleAdaptingABCDistance2017}. 
...
The exact configurations of PMC-ABC runs are given in the calibration script files.\\
%
...
The corresponding parameter values are reported in Table \ref{tab:params_24d}.
...
\begin{table}[h]
    ...
    \label{tab:params_24d}
    \begin{tabularx}{\textwidth}{lllll}
        \hline
        Parameter & Best fit & Median & $P_{5}$ & $P_{95}$ \\ 
        $k_{D}$ ($d^{-1}$) & 0.97 & 0.92 & 0.76 & 0.99 \\
...
The corresponding parameter values are reported in Table \ref{tab:params_flp}.
...
\begin{table}[h]
    ...
    \label{tab:params_flp}
    \begin{tabularx}{\textwidth}{lllll}
        \hline
        Parameter & Best fit & Median & $P_{5}$ & $P_{95}$ \\ 
        $k_{D}$ ($d^{-1}$) & 0.97 & 0.31 & 0.12 & 0.93 \\
...
mixture effects on the timing of GS 46 with high accuracy, but low precision (Figure \ref{fig:mixture_metamorphs}). 
This coincided with a high mortality at the corresponding treatments. 
The model predictions imply that individuals, if they would not have died from the exposure, 
...
one of the goals of the amphibian DEB-TKTD model development is to lay out the basis to extrapolate effects to the population level.
...
may translate to lethal effects in the field.\\
...
SH - Formal analysis, Investigation, Methodology, Software, Validation, Visualization, Writing - original draft;
SGL - Data curation, resources;
MOS - Funding acquisition, Project administration, Resources, Supervision, Writing - review \& editing;
JLD - Conceptualization, Funding acquisition, project administration, supervision
AF - Funding acquisition, Project administration, Resources, Supervision, Writing - review \& editing;

\section*{Funding}
This work was part of the AmphiDEB project fundend by the European Food Safety Authority (Ref. OC/EFSA/SCER/2021/12).

\section*{Acknowledgments}
We thank Sandrine Charles for fruitful discussion of the modelling approach.

\section*{Supplemental Data}
The supplemental data includes all code and data used for this study.

\section*{Data Availability Statement}
The datasets anylzed in this study are included in the code repository (Supplemental DAta).
