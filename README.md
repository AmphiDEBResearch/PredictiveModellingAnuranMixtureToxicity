# Calibration and Validation of TKTD models for the anuran *Discoglossus galganoi* 

Organization of this project:

- assets: any additional files like icons and images 
- data: all inputs, including experimental data and parameters
- lib: locally installed packages
- plots: plots produced by scripts
- scripts: all script files

>[!NOTE] To reproduce the results, make sure that the Julia version listed in Manifest.toml is installed and activated (juliaup is recommended). 
> Unregistered Julia packages are added as git submodules under `lib/.`. After cloning this repo, use `git submodule update --init --recursive`.