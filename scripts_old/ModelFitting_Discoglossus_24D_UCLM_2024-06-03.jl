...
function plot_data(;kwargs...)
    ...
    plt_aqua = @df f.data[:aquatic] plot(
        ...
        groupedlineplot(
            :t_exp, :num_tadpoles, :C_W_1, layout = (1,length(unique(:C_W_1))), leg = false, 
            fillalpha = .2, lw = 2, color = :black, marker = true, ylabel = ["Number of tadpoles \n per aquarium" "" "" "" ""],
            xlim = (-5, 30), xlabel = "Time (d)",  
        ), 
    ...
    for (i,C_W) in enumerate(unique(f.data[:aquatic].C_W_1))
        ...
        @df df lineplot!(plt_aqua, :t_exp, :num_tadpoles, (.5, .5), subplot = c+num_concs, color = :gray, lw = 2, linestyle = :dash)
    end
...
function plot_sims!(plt, predictions::AbstractVector; label = "Simulation", color = :steelblue)::Nothing
    ...
    for (i,C_W) in enumerate(unique(df_aqua.C_W_1))
        ...
        @df df lineplot!(plt, :t_exp, :num_tadpoles, subplot = c+num_concs, color = color, lw = 2, fillalpha = .2)
    end
...
function setup_modelfit(pmoa::AbstractString; σ_factor = 2.)
    ...
    f = ModelFit( 
        prior = Prior(
            ...
            "spc.E[1,$(pmoa_idx)]" => Truncated(Normal(15, 15*σ_factor), 0.03, Inf),
...
# TODO: implement automatic display + saving of quant eval table after calibration
function quant_eval(
    f::ModelFit,
    predictions::Vector{OrderedDict},
    )
